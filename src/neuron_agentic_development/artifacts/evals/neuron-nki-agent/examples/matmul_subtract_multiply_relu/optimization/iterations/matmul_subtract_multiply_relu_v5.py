"""
Fused matmul + subtract + multiply + ReLU NKI kernel — V5 optimization.

V5: Fuse PSUM→SBUF copy + bias add into single tensor_tensor instruction.
  Old: tensor_copy(psum→sbuf) + tensor_tensor(sbuf+bias) = 2 instructions
  New: tensor_tensor(psum+bias_all → sbuf) = 1 instruction
Then fused activation for subtract+multiply+relu.
Total epilogue: 2 instructions instead of 4.

Built on V4's full data hoisting (x_t outside all loops, weight per-N).
...
def matmul_subtract_multiply_relu_kernel(
    ...
    """V5: Fused PSUM copy + bias add, full data hoisting."""
    ...
    # Full x_t hoisting: load ALL x_t tiles before any compute (2MB SBUF)
    all_lhsT = []
    ...
    for n_idx in nl.affine_range(num_n_tiles):
        ...
        # Bias hoisting: broadcast once per N
        ...
        for m_idx in nl.affine_range(num_m_tiles):
            ...
            # === FUSED EPILOGUE ===
            # Step 1: Copy PSUM + add bias in ONE instruction
            # tensor_tensor reads PSUM (data1) and SBUF (data2), writes SBUF (dst)
            result_sb = nl.ndarray((P_MAX, TILE_N), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(dst=result_sb, data1=psum_tile, data2=bias_all, op=nl.add)

            # Step 2: Fused subtract + multiply + ReLU in ONE activation
            ...
            # Store
            nisa.dma_copy(
                dst=output[m_idx * P_MAX:m_idx * P_MAX + P_MAX, n_start:n_start + TILE_N],
                src=result_sb
            )
