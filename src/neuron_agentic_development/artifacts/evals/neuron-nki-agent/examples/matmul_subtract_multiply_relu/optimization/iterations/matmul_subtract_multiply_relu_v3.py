"""
Fused matmul + subtract + multiply + ReLU NKI kernel — V3 optimization.

V3: Fuse epilogue — combine subtract+multiply+relu into single nisa.activation.
  relu((result + bias - sub) * mul) = relu(result_with_bias * mul - sub * mul)
  = activation(data=result_with_bias, op=relu, scale=mul, bias=-sub*mul)
This eliminates one tensor_scalar instruction per tile.

Built on V2's N-outer/M-inner loop with weight and bias hoisting.
...
def matmul_subtract_multiply_relu_kernel(
    ...
    """V3: Fused epilogue — activation(scale, bias) replaces tensor_scalar + activation."""
    ...
    # Precompute fused bias: -subtract_value * multiply_value
    ...
    for n_idx in nl.affine_range(num_n_tiles):
        ...
        # Weight hoisting: load all K tiles for this N
        rhs_tiles = []
        ...
        # Bias hoisting: load and broadcast once per N tile
        ...
        for m_idx in nl.affine_range(num_m_tiles):
            ...
            # Load all K tiles of x_t for this (N, M) pair
            lhsT_tiles = []
            for k_idx in nl.affine_range(num_k_tiles):
                k_start = k_idx * TILE_K
            ...
            # Epilogue: PSUM→SBUF + add bias + fused (subtract*multiply+relu)
            ...
            # Fused: relu(result_with_bias * multiply_value + (-subtract * multiply))
            # = relu((result_with_bias - subtract) * multiply)
