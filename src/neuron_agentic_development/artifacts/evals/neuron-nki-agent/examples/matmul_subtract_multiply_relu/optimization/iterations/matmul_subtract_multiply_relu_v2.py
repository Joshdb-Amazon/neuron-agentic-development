"""
Fused matmul + subtract + multiply + ReLU NKI kernel — V2 optimization.

V2: N-outer/M-inner loop order with weight hoisting.
- Weight tiles loaded once per N tile, reused across M tiles (32MB vs 64MB)
- x_t tiles loaded per (N, M) pair (16MB vs 2MB — trades off for weight savings)
- Net: ~48MB HBM reads vs V1's ~66MB (27% reduction)
...
def matmul_subtract_multiply_relu_kernel(
    ...
    for n_idx in nl.affine_range(num_n_tiles):
        ...
        # === WEIGHT HOISTING: Load all K tiles of weight for this N tile ===
        # Reused across all M iterations (2x in this case)
        # SBUF: 32 tiles * 128*512*2 = 4MB
        rhs_tiles = []
        ...
        # Pre-load bias for this N tile (reused across M)
