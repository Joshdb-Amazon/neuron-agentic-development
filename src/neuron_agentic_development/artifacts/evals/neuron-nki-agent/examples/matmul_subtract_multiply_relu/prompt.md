Write a NKI kernel that runs on a single neuron core for the matmul_subtract_multiply_relu.py, make sure to run it on device and compare with torch CPU baseline to ensure correctness. Use the neuron-nki-writing skill. Also, use the neuron-nki-docs skill to look up NKI docs for API usage, architecture constraints, etc.
