# Matmul (with B^T) — Optimization

You are optimizing the `matmul_bt` kernel through 5 versioned improvements.

- **Baseline kernel**: `evals/neuron-nki-agent/examples/matmul/output/matmul_bt_kernel.py`
- **Baseline test**: `evals/neuron-nki-agent/examples/matmul/output/test_matmul_bt.py`
- **Working directory**: `evals/neuron-nki-agent/examples/matmul/optimization/tmp/`
- **Final output**: `evals/neuron-nki-agent/examples/matmul/optimization/output/`
...
You are assigned neuron cores **12–15** (4 cores).
Set `NEURON_RT_VISIBLE_CORES=12-15` in every compile and run command.
...
3. **Design the optimization** — consult `/neuron-nki-optimizing` for strategies that
   ...
   patterns.
4. **Implement** as `matmul_bt_v<N>.py` in the working directory.
...
Only write to `evals/neuron-nki-agent/examples/matmul/optimization/output/` when completely done:
...
- `matmul_bt_v5.py` — your best optimized kernel (or latest working version)
- `test_matmul_bt.py` — correctness test covering baseline and optimized
...
- `NEURON_RT_VISIBLE_CORES=12-15` on every compile/run
- Use `evals/neuron-nki-agent/examples/matmul/optimization/tmp/` for ALL intermediate work
- Only write final deliverables to `evals/neuron-nki-agent/examples/matmul/optimization/output/`
- Do NOT modify the original kernel or test files in `evals/neuron-nki-agent/examples/matmul/output/`
