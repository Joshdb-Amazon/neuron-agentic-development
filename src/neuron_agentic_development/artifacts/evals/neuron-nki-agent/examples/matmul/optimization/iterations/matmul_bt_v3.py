...
V3: Increase M blocking to TILES_IN_BLOCK_M=8.
    A tiles: 8×64×32KB = 16 MB, B tiles: 64×32KB = 2 MB = 18 MB total SBUF.
    B reloads reduced from 4x (V2) to 2x. Expected HBM reads: ~160 MB.
"""
...
def matmul_bt_kernel(A, B):
    ...
    num_m_blocks = M // BLOCK_M  # 2048/1024 = 2
    ...
    for m_blk in nl.affine_range(num_m_blocks):
        ...
        # 8 M tiles × 64 K tiles = 512 tiles × 32KB = 16 MB SBUF
        A_tiles = []
        ...
        for n_idx in nl.affine_range(num_n_tiles):
            ...
            # Compute matmul for all 8 M tiles × this N tile
            for bm in nl.affine_range(TILES_IN_BLOCK_M):
                m_idx = m_blk * TILES_IN_BLOCK_M + bm
