# MLP (SwiGLU) — Optimization

You are optimizing the `swiglu_mlp` kernel through 5 versioned improvements.

- **Baseline kernel**: `evals/neuron-nki-agent/examples/mlp/output/swiglu_mlp_kernel.py`
- **Baseline test**: `evals/neuron-nki-agent/examples/mlp/output/test_swiglu_mlp.py`
- **Working directory**: `evals/neuron-nki-agent/examples/mlp/optimization/tmp/`
- **Final output**: `evals/neuron-nki-agent/examples/mlp/optimization/output/`
...
You are assigned neuron cores **20–23** (4 cores).
Set `NEURON_RT_VISIBLE_CORES=20-23` in every compile and run command.
...
3. **Design the optimization** — consult `/neuron-nki-optimizing` for strategies that
   ...
   patterns.
4. **Implement** as `swiglu_mlp_v<N>.py` in the working directory.
...
Only write to `evals/neuron-nki-agent/examples/mlp/optimization/output/` when completely done:
...
- `swiglu_mlp_v5.py` — your best optimized kernel (or latest working version)
- `test_swiglu_mlp.py` — correctness test covering baseline and optimized
...
- `NEURON_RT_VISIBLE_CORES=20-23` on every compile/run
- Use `evals/neuron-nki-agent/examples/mlp/optimization/tmp/` for ALL intermediate work
- Only write final deliverables to `evals/neuron-nki-agent/examples/mlp/optimization/output/`
- Do NOT modify the original kernel or test files in `evals/neuron-nki-agent/examples/mlp/output/`
