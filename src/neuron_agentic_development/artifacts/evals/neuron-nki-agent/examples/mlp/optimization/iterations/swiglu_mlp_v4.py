"""
SwiGLU MLP NKI Kernel - v4: Batch token tiles for weight reuse.

Optimization over v2: Process token tiles in batches of 2. For each batch:
1. Load x_t tiles for both token tiles into SBUF (cached across h_idx)
2. Initialize output accumulators for both token tiles in SBUF
3. For each h_idx: load weights once, compute for both token tiles
4. Write accumulators to HBM at the end of the batch

This halves weight loading (16 batches × 192MB vs 32 × 192MB) while
x_t tiles stay cached (no extra reloading) and output accumulates in
float32 SBUF (no HBM R/W per h_idx).
...
F_STAT_MAX = 128
F_MOV_MAX = 512
BATCH_T = 2  # Number of token tiles to process per batch
...
def swiglu_mlp_kernel(
    ...
    v4: Batched token tiles for weight reuse
    """
    ...
    num_batches = div_ceil(num_token_tiles, BATCH_T)

    for batch_idx in nl.affine_range(num_batches):
        # === Load x_t tiles for both token tiles in this batch ===
        # These persist across all h_idx iterations
        batch_x_t_tiles = []  # [t_local][k_idx]
        batch_t_starts = []
        batch_t_sizes = []

        for t_local in range(BATCH_T):
            t_idx = batch_idx * BATCH_T + t_local
            t_start = t_idx * F_STAT_MAX
            t_end = min(t_start + F_STAT_MAX, tokens)
            t_size = t_end - t_start

            if t_start >= tokens:
                # Padding batch - won't be used, but allocate anyway
                batch_t_starts.append(0)
                batch_t_sizes.append(0)
                batch_x_t_tiles.append([])
                continue

            batch_t_starts.append(t_start)
            batch_t_sizes.append(t_size)
            ...
            batch_x_t_tiles.append(x_t_tiles)

        # === Initialize output accumulators for both token tiles ===
        batch_out_accum = []  # [t_local][o_idx]
        for t_local in range(BATCH_T):
            t_size = batch_t_sizes[t_local]
            if t_size == 0:
                batch_out_accum.append([])
                continue
            out_accum = []
            ...
            batch_out_accum.append(out_accum)

        # === For each hidden tile: load weights once, process both token tiles ===
        for h_idx in nl.sequential_range(num_hidden_tiles):
            ...
            # Load gate and up weight tiles (shared between both token tiles)
            gate_w_tiles = []
            up_w_tiles = []
            for k_idx in nl.affine_range(num_k_in_tiles):
                k_start = k_idx * P_MAX
            ...
            # Load down weight tiles (shared between both token tiles)
            down_w_tiles = []  # [k_sub][o_idx]
            ...
            # Process each token tile in the batch
            for t_local in range(BATCH_T):
                t_size = batch_t_sizes[t_local]
                t_start = batch_t_starts[t_local]

                if t_size == 0:
                    continue

                # Gate and Up projections using cached weights and x_t tiles
                ...
                for k_idx in nl.affine_range(num_k_in_tiles):
                    k_start_k = k_idx * P_MAX
                    k_end_k = min(k_start_k + P_MAX, input_size)
                    k_size = k_end_k - k_start_k

                    nisa.nc_matmul(
                        dst=gate_psum[0:t_size, 0:h_size],
                        stationary=batch_x_t_tiles[t_local][k_idx][0:k_size, 0:t_size],
                ...
                # Down projection with cached down weights
                for k_sub_idx in nl.affine_range(num_k_sub_actual):
                    ks_start = k_sub_idx * P_MAX
                    ks_end = min(ks_start + P_MAX, h_size)
                    ks_size = ks_end - ks_start
                    ...
                    for o_idx in nl.affine_range(num_out_tiles):
                        ...
                        nisa.tensor_tensor(
                            dst=batch_out_accum[t_local][o_idx][0:t_size, 0:o_size],
                            data1=batch_out_accum[t_local][o_idx][0:t_size, 0:o_size],
                            data2=down_psum[0:t_size, 0:o_size],
                            op=nl.add,
                        )

        # === Write all accumulators to HBM ===
        for t_local in range(BATCH_T):
            t_size = batch_t_sizes[t_local]
            t_start = batch_t_starts[t_local]
            ...
            for o_idx in nl.affine_range(num_out_tiles):
                ...
                nisa.tensor_copy(
                    ...
                    src=batch_out_accum[t_local][o_idx][0:t_size, 0:o_size],
                )

                nisa.dma_copy(
                    dst=output[t_start:t_start + t_size, o_start:o_end],
