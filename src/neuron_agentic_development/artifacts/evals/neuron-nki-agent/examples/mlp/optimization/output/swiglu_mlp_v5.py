"""
SwiGLU MLP NKI Kernel - v5: Increased batch size to 4 token tiles.

Optimization over v4: Increase BATCH_T from 2 to 4. This reduces weight
loading by 4x vs v2 (8 batches × 192MB vs 32 × 192MB = 1536 vs 6144 MB).

SBUF budget: x_t(4MB) + accum(8MB) + weights(12MB) + work(1MB) = 25MB.
Tight fit but compiler should manage with SBUF spilling being minimal.
...
F_STAT_MAX = 128
F_MOV_MAX = 512
BATCH_T = 4  # Number of token tiles to process per batch
...
def swiglu_mlp_kernel(
    ...
    v5: Larger batch (4) for more weight reuse
    """
