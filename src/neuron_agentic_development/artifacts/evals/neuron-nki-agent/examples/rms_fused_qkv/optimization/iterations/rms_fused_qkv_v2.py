"""
Fused RMSNorm + QKV projection NKI kernel — v2 optimization.

v2 changes vs v1:
- Compute inv_rms incrementally per K-chunk instead of loading full [128,2048]
  input. This eliminates the large input_sb [128,2048] and sq_sb [128,2048]
  buffers (~2MB SBUF savings), potentially enabling better compiler scheduling.
- Each K-chunk is loaded from HBM as [128,128] for RMSNorm (square + reduce).
- The matmul still uses dma_transpose from HBM (v1 approach).
- Total HBM reads should stay similar since the input chunks for RMSNorm overlap
  with the dma_transpose reads (same HBM regions).
"""
