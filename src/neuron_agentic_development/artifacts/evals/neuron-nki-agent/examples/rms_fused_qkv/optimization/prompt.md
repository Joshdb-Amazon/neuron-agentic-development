# RMSNorm + Fused QKV — Optimization

You are optimizing the `rms_fused_qkv` kernel through 5 versioned improvements.

- **Baseline kernel**: `evals/neuron-nki-agent/examples/rms_fused_qkv/output/rms_fused_qkv_kernel.py`
- **Baseline test**: `evals/neuron-nki-agent/examples/rms_fused_qkv/output/test_rms_fused_qkv.py`
- **Working directory**: `evals/neuron-nki-agent/examples/rms_fused_qkv/optimization/tmp/`
- **Final output**: `evals/neuron-nki-agent/examples/rms_fused_qkv/optimization/output/`
...
You are assigned neuron cores **24–27** (4 cores).
Set `NEURON_RT_VISIBLE_CORES=24-27` in every compile and run command.
...
3. **Design the optimization** — consult `/neuron-nki-optimizing` for strategies that
   ...
   patterns.
4. **Implement** as `rms_fused_qkv_v<N>.py` in the working directory.
...
Only write to `evals/neuron-nki-agent/examples/rms_fused_qkv/optimization/output/` when completely done:
...
- `rms_fused_qkv_v5.py` — your best optimized kernel (or latest working version)
- `test_rms_fused_qkv.py` — correctness test covering baseline and optimized
...
- `NEURON_RT_VISIBLE_CORES=24-27` on every compile/run
- Use `evals/neuron-nki-agent/examples/rms_fused_qkv/optimization/tmp/` for ALL intermediate work
- Only write final deliverables to `evals/neuron-nki-agent/examples/rms_fused_qkv/optimization/output/`
- Do NOT modify the original kernel or test files in `evals/neuron-nki-agent/examples/rms_fused_qkv/output/`
