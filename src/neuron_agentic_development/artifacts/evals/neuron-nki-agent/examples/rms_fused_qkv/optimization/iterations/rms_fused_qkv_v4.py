"""
Fused RMSNorm + QKV projection NKI kernel — v4 optimization.

v4 changes vs v2/v3:
- Minimize SBUF pressure in RMSNorm: square in-place (no separate chunk_sq),
  use nl.loop_reduce for accumulation (compiler hint for loop optimization).
- Fuse PSUM copy + inv_rms multiply (v3 approach) into single tensor_tensor.
- These SBUF pressure reductions may enable better DMA-compute overlap.
"""
...
def rms_fused_qkv_kernel(hidden_hbm, weight_hbm, eps=1e-6):
    ...
    for s_idx in nl.affine_range(num_seq_tiles):
        ...
        # ========== Phase 1: Compute inv_rms with minimal SBUF ==========
        ...
        for k_idx in nl.affine_range(num_k_tiles):
            ...
            # Partial reduce into chunk's first column (reuse buffer)
        ...
        for n_idx in nl.affine_range(num_n_tiles):
            ...
            # Fused: PSUM × inv_rms → SBUF (no separate tensor_copy)
