"""
NKI Softmax Kernel v3 - Fuse exp + sum_reduce into single activation instruction.

Optimization over v2:
  - Use nisa.activation with reduce_op=nl.add and reduce_cmd=reset_reduce
    to compute exp(x-max) AND sum(exp(x-max)) in a single instruction pass.
  - This eliminates the separate tensor_reduce(sum) call entirely.
  - Combined with v1's subtract+exp fusion, the entire softmax computation
    now has fewer instruction passes over the data.
...
def softmax_kernel(input_tensor: nl.ndarray) -> nl.ndarray:
    ...
    for r_idx in nl.affine_range(num_row_tiles):
        ...
        # --- Step 3 (FUSED): exp(input - max) AND sum(exp(input - max)) ---
        # Single activation instruction that:
        #   1. Computes exp(tile * 1.0 + (-max)) = exp(tile - max) -> stored in tile
        #   2. Reduces sum(exp(tile - max)) along free axis -> stored in sum_sb
        ...
        # --- Step 4: Compute reciprocal of sum ---
