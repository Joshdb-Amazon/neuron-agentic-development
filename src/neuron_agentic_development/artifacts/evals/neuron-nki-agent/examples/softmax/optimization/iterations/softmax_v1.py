"""
NKI Softmax Kernel v1 - Fuse subtract+exp into single activation instruction.

Optimization: Replace separate tensor_scalar(subtract) + activation(exp) with
a single nisa.activation(op=exp, bias=-max) that computes exp(data*1.0 + (-max)).
This eliminates the intermediate shifted_sb buffer and reduces instruction count.
...
def softmax_kernel(input_tensor: nl.ndarray) -> nl.ndarray:
    ...
    for r_idx in nl.affine_range(num_row_tiles):
        ...
        # --- Step 2: Negate max for use as bias in fused activation ---
        ...
        # --- Step 3 (FUSED): Compute exp(input - max) = exp(input * 1.0 + (-max)) ---
        # Fuses the subtract + exp into a single activation instruction
