...
You are optimizing the `softmax` kernel through 5 versioned improvements.

- **Baseline kernel**: `evals/neuron-nki-agent/examples/softmax/output/softmax_nki_kernel.py`
- **Baseline test**: `evals/neuron-nki-agent/examples/softmax/output/test_softmax.py`
- **Working directory**: `evals/neuron-nki-agent/examples/softmax/optimization/tmp/`
- **Final output**: `evals/neuron-nki-agent/examples/softmax/optimization/output/`
...
You are assigned neuron cores **28–31** (4 cores).
Set `NEURON_RT_VISIBLE_CORES=28-31` in every compile and run command.
...
3. **Design the optimization** — consult `/neuron-nki-optimizing` for strategies that
   ...
   patterns.
4. **Implement** as `softmax_v<N>.py` in the working directory.
...
Only write to `evals/neuron-nki-agent/examples/softmax/optimization/output/` when completely done:
...
- `softmax_v5.py` — your best optimized kernel (or latest working version)
- `test_softmax.py` — correctness test covering baseline and optimized
...
- `NEURON_RT_VISIBLE_CORES=28-31` on every compile/run
- Use `evals/neuron-nki-agent/examples/softmax/optimization/tmp/` for ALL intermediate work
- Only write final deliverables to `evals/neuron-nki-agent/examples/softmax/optimization/output/`
- Do NOT modify the original kernel or test files in `evals/neuron-nki-agent/examples/softmax/output/`
