"""
Conv2D + Scale + Min NKI Kernel - v3: Batch output stores

Optimizations over v2:
1. Accumulate all row results into a contiguous SBUF buffer, then do
   ONE large DMA store per batch instead of 62 tiny (124-byte) stores.
   This eliminates the DMA overhead of 496 tiny stores.
2. Keeps weight hoisting and fused scale+negate from v1/v2
"""
...
def conv2d_scale_min_v3(
    ...
    for b_idx in nl.affine_range(batch_size):
        ...
        # OPTIMIZATION: Accumulate all output rows into one contiguous buffer
        # Then do a single large DMA store instead of 62 tiny stores
        output_buf = nl.ndarray((1, n_out), dtype=input_hbm.dtype, buffer=nl.sbuf)
        ...
        for h_o in nl.affine_range(h_out):
            ...
            # Negate back and store into output buffer
            out_row_start = h_o * w_out
            nisa.tensor_scalar(
                dst=output_buf[0:1, out_row_start:out_row_start+w_out],
                data=max_neg[0:1, 0:w_out],
                op0=nl.multiply,
                operand0=-1.0
            )

        # Single large DMA store for all rows at once
        nisa.dma_copy(
            dst=output_hbm[b_idx, 0:n_out],
            src=output_buf[0, 0:n_out]
        )
