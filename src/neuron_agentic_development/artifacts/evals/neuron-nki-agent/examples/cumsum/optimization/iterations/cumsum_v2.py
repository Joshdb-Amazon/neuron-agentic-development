"""
NKI Cumsum Kernel - V2: Double buffering + store overlap

Building on V1's double buffering, this version also overlaps the store of the
previous tile's result with the current tile's scan. The DMA engine can handle
the store while the vector engine computes the scan.

Also explores whether the compiler can better pipeline when stores are issued
before the scan (store previous result, then scan current data).
"""
...
def cumsum_kernel(x: nl.ndarray) -> nl.ndarray:
    ...
    for p_tile in TiledRange(outer_dim, P_MAX):
        ...
        # Triple-buffer: 2 data + 2 result buffers for full pipeline overlap
        ...
        for f_tile_idx in nl.sequential_range(num_f_tiles):
            ...
            if f_tile_idx % 2 == 0:
                ...
                nxt_data = data_sb_1
                prev_result = result_sb_1
            else:
                cur_data = data_sb_1
                cur_result = result_sb_1
                nxt_data = data_sb_0
                prev_result = result_sb_0

            # Issue DMA operations first (both load next + store previous)
            # so they can overlap with the scan below
            ...
            # Store previous result (overlap with current scan)
            if f_tile_idx > 0:
                prev_f_start = (f_tile_idx - 1) * F_TILE_SIZE
                prev_f_end = min(prev_f_start + F_TILE_SIZE, last_dim)
                prev_f_size = prev_f_end - prev_f_start
                nisa.dma_copy(
                    dst=y[p_tile.start_offset:p_tile.start_offset + p_tile.size, prev_f_start:prev_f_end],
                    src=prev_result[0:p_tile.size, 0:prev_f_size],
                )

            # Scan current tile (Vector engine, overlaps with DMA above)
        ...
        last_f_end = min(last_f_start + F_TILE_SIZE, last_dim)
        last_f_size = last_f_end - last_f_start
        if num_f_tiles % 2 == 0:
            last_result = result_sb_1
        else:
            last_result = result_sb_0
        nisa.dma_copy(
            dst=y[p_tile.start_offset:p_tile.start_offset + p_tile.size, last_f_start:last_f_end],
            src=last_result[0:p_tile.size, 0:last_f_size],
        )
