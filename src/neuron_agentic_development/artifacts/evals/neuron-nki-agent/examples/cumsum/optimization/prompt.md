...
You are optimizing the `cumsum` kernel through 5 versioned improvements.

- **Baseline kernel**: `evals/neuron-nki-agent/examples/cumsum/output/cumsum_kernel.py`
- **Baseline test**: `evals/neuron-nki-agent/examples/cumsum/output/test_cumsum.py`
- **Working directory**: `evals/neuron-nki-agent/examples/cumsum/optimization/tmp/`
- **Final output**: `evals/neuron-nki-agent/examples/cumsum/optimization/output/`
...
You are assigned neuron cores **4–7** (4 cores).
Set `NEURON_RT_VISIBLE_CORES=4-7` in every compile and run command.
...
3. **Design the optimization** — consult `/neuron-nki-optimizing` for strategies that
   ...
   patterns.
4. **Implement** as `cumsum_v<N>.py` in the working directory.
...
Only write to `evals/neuron-nki-agent/examples/cumsum/optimization/output/` when completely done:
...
- `cumsum_v5.py` — your best optimized kernel (or latest working version)
- `test_cumsum.py` — correctness test covering baseline and optimized
...
- `NEURON_RT_VISIBLE_CORES=4-7` on every compile/run
- Use `evals/neuron-nki-agent/examples/cumsum/optimization/tmp/` for ALL intermediate work
- Only write final deliverables to `evals/neuron-nki-agent/examples/cumsum/optimization/output/`
- Do NOT modify the original kernel or test files in `evals/neuron-nki-agent/examples/cumsum/output/`
