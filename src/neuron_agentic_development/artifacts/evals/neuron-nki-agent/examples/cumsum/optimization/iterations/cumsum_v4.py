"""
NKI Cumsum Kernel - V4: Double buffering + smaller tiles (F_TILE_SIZE=1024)

Hypothesis: Smaller tiles allow more frequent interleaving of DMA and compute.
Each scan is shorter, so the DMA engine can start loading/storing sooner.
With 32 sequential tiles per partition (vs 16 baseline), there are more
pipeline stages to overlap.

Also uses a smaller ones_sb buffer (saves 512KB SBUF), freeing space for
the compiler to schedule DMA more freely.
"""
...
def cumsum_kernel(x: nl.ndarray) -> nl.ndarray:
    ...
    # V4: Smaller tiles for better pipelining
    F_TILE_SIZE = min(last_dim, 1024)
