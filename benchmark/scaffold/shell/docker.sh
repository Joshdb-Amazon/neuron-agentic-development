...
# Usage: build_env_args; docker run ${ENV_ARGS[@]+"${ENV_ARGS[@]}"} ...
build_env_args() {
    ENV_ARGS=()
...
docker_run_kiro() {
    ...
    # Mount kiro auth: either KIRO_API_KEY env var or SSO cache from host
    local auth_args=()
    if [[ -z "${KIRO_API_KEY:-}" ]]; then
        # Mount host auth database for SSO credentials
